...
class SignUpdater extends Task
{
    ...
    public function onRun(int $currentTick) : void
    {
        foreach ($this->plugin->signs as $arena => $positions) {
            foreach ($positions as $position) {
                ...
                $tile->setText(TextFormat::BOLD . TextFormat::YELLOW . "BedWars",
                TextFormat::BOLD . TextFormat::AQUA . $game->getMapName(),
                    TextFormat::GREEN . count($game->players) . "§7/§a" . $game->getMaxPlayers(),
    ...
    public function getStatus(int $state) : string{
        switch($state){
            case 0;
                return TextFormat::WHITE . "Touch Me";
            case 1;
